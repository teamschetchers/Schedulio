...
const Bookings = () => {
  ...
  const bookings = useSelector((state: any) => state?.admin?.bookings);

  const user = useSelector((state: any) => state?.login?.userInfo);
  console.log("bookings", bookings);

  useEffect(() => {
    dispatch(getAllUserBookings(user._id) as any);
  }, []);
  ...
  const changeBookingStatus = (serviceId, status) => {
    ...
    const res = booking?.filter((booking) => booking._id !== serviceId);
    setBookings(res);
    ...
    <>
      ...
      >
        ...
        <div className="d-flex justify-content-center align-items-center">
          <Link to="/">
            <Button type="submit" variant={`dark`} onClick={() => console.log}>
              Add Reservation
            </Button>
          </Link>
        </div>
